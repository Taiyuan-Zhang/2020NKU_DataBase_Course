...
public class BTreeReverseScanTest extends SimpleDbTestBase {
    ...
    private void validateScan(int[] columnSizes, int[] rowSizes)
        ...
        for (int i = columnSizes.length-1; i>=0; i--) {
            int keyField = r.nextInt(columnSizes[i]);
            for (int j = rowSizes.length-1; j>=0; j--) {
                ArrayList<ArrayList<Integer>> tuples = new ArrayList<ArrayList<Integer>>();
                BTreeFile f = BTreeUtility.createRandomBTreeFile(columnSizes[i], rowSizes[j], null, tuples, keyField);
